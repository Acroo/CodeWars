...
public class RainfallKata {

    public static double mean(String town, String data){
        double retValue = -1;
        double total = 0;
        if(!town.isEmpty() && !data.isEmpty()) {
            Map<String, Double> dataMap = stringToMap(findTownData(town, data));
            for (Map.Entry<String, Double> entry : dataMap.entrySet()) {
                total += entry.getValue().doubleValue();
            }
            retValue = total/dataMap.size();
        }
    ...
    public static double variance(String town, String data){
        ...
        if(!town.isEmpty() && !data.isEmpty()) {
            Map<String, Double> dataMap = stringToMap(findTownData(town, data));
            double mean = RainfallKata.mean(town,data);

            for(double a : dataMap.values())
                retValue += (a-mean)*(a-mean);
            retValue = retValue/(dataMap.size()-1);
        }
        return retValue;
    }
    ...
    public static Map<String, Double> stringToMap(String str) {
        ...
        for (int i=0; i<tokens.length; ){
            ...
            map.put(innerTokens[0], Double.valueOf(innerTokens[1]));
        }
        return map;
    }

    public static String findTownData(String town, String data) {
        String ret =  new String();
        StringTokenizer st = new StringTokenizer(data, "\n");
        while(st.hasMoreTokens() && ret.isEmpty()){
            String tmp = st.nextToken();
            if(tmp.startsWith(town)) ret = tmp;
        }
